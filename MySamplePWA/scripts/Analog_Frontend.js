    ...
    if (canvas.getContext) {
        ...
        if(analog_stick_flag){
            ...
            if(store_val_flag){
                ...
                ctx.arc(analog_stick_X, analog_stick_Y, R-15, 0, Math.PI * 2, true); // Outer circle
                ctx.stroke();
                drawReadingText(ctx,"X = " + Math.round(mapStickVals(Curr_AX_Cal_Vals, currentAX,0,AnalogStickXDeadzone,1)).toString(),oX+R-25,oY+R+40);
                drawReadingText(ctx,"Y = " + Math.round(mapStickVals(Curr_AY_Cal_Vals, currentAY,1,AnalogStickYDeadzone,1)).toString(),oX+R-25,oY+R+70);
            }
        }
...
function bisect1(a, val, size, dir_flag) {
    var lo = 0;
    var hi = size;
    var mid;
...
function bisect2(a, val, size, dir_flag) {
    if(!dir_flag){
        var lo = 0;
        var hi = size;
        var mid;
...
*/
function mapStickVals_old_old(calArray, val, XorY, dead, printFlag=0){
    var mapped_val = 127;
    var flipped_magnet_dir;
    var a = 0;
    var b = 1;
    var min_val = 0;
    var max_val = 1;

    var sorted_cals = calArray;

    const numCalibPoints = 7;
    ...
    var index = bisect2(calArray, val, numCalibPoints, flipped_magnet_dir);
 
    if(printFlag == 1){
        console.log("val = " + val);
        console.log("Calib array = " + sorted_cals);
    ...
    if (index == 0){
        ...
        min_val = sorted_cals[0];
        max_val = sorted_cals[1];
        ...
        min_val = sorted_cals[2];
        max_val = sorted_cals[3];
        mapped_val = (a + (val - min_val)*(b-a) / (max_val - min_val));
    }
    else if (index == 4){
        a = 127;
        ...
        min_val = sorted_cals[3];
        max_val = sorted_cals[4];
        mapped_val = (a + (val - min_val)*(b-a) / (max_val - min_val));
    }
    else if (index == 5){
        mapped_val = 218;
        ...
        min_val = sorted_cals[5];
        max_val = sorted_cals[6];
        mapped_val = (a + (val - min_val)*(b-a) / (max_val - min_val));
    }
